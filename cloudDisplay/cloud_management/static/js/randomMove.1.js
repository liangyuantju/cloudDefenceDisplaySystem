...
window.onload = function() {
	...
	var timer = setInterval( function() {
		...
		// box.innerHTML = '';
		// sc = new Screen('moveBox', { ballsnum: 2*openNum, spring: 0.3, bounce: -0.9, gravity: 0.05 });
		// sc.initialize();
		allFunc();
	}, 3000);
...
function allFunc() {
	...
	var cloudnNum = Number(window.localStorage.getItem("csNumber"));
	var openNum = Number(window.localStorage.getItem("osNumber"));
	//var cureCloud = 1;
	var clss = new Array();
	for (let i = 1; i <= cloudnNum; i++) {
		var clssName = 'cloud0' + i;
		clss.push(clssName);
	}
	for (let i = 1; i <= openNum; i++) {
		var clssName = 'open0' + i;
		clss.push(clssName);
	}
	/*
	for (let i = 1; i <= cureCloud; i++) {
		clss.push('cloud01');
	}
	*/
	//var clss = ['cloud01', 'cloud02', 'cloud03', 'cloud04', 'cloud05', 'cloud06'];
	...
	var box = document.getElementById("moveBox");
	box.innerHTML = '';
	sc = new Screen('moveBox', { ballsnum: cloudnNum + openNum, spring: 0.3, bounce: -0.9, gravity: 0.05 });
	sc.initialize();
